...
static int8_t xgb_prepare_individual_write_frame(
  ...
  temp_frame.ind_write_frame.station_number[0] =
      (frame_data.ind_write.station_number / 16) + '0';
  ...
  temp_frame.ind_write_frame.no_blocks[0] =
      (frame_data.ind_write.no_of_blocks / 16) + '0';
  ...
  temp_frame.ind_write_frame.device_lenght[0] =
      ((3 + strlen(frame_data.ind_write.p_device_address)) / 16) + '0';
  ...
  temp_frame.ind_write_frame.device_name[1] =
      frame_data.ind_write.device_type;  // device memory group (P,M,L etc.)
  temp_frame.ind_write_frame.device_name[2] = frame_data.ind_write.data_size;
  strcpy((char *restrict)(&temp_frame.ind_read_frame.device_name[3]),
         (const char *)frame_data.ind_write.p_device_address);

  // prepare frame data
  uint8_t no_bytes_to_copy =
      frame_data.ind_write.no_of_blocks *
      (data_marking_to_size(frame_data.ind_write.data_size) * 2);
  memcpy(temp_frame.ind_write_frame.data, frame_data.ind_write.p_data_buffer,
         no_bytes_to_copy);
...
static int8_t xgb_prepare_continuous_read_frame(
  ...
  temp_frame.cont_read_frame.station_number[0] =
      (frame_data.cont_read.station_number / 16) + '0';
  ...
  temp_frame.cont_read_frame.device_lenght[0] =
      ((3 + strlen(frame_data.cont_read.p_device_address)) / 16) + '0';
  ...
  temp_frame.cont_read_frame.device_name[1] =
      frame_data.cont_read.device_type;  // device memory group (P,M,L etc.)
  temp_frame.cont_read_frame.device_name[2] = frame_data.cont_read.data_size;
  strcpy((char *restrict)(&temp_frame.cont_read_frame.device_name[3]),
         (const char *)frame_data.cont_read.p_device_address);

  // no of data
  temp_frame.cont_read_frame.no_data[0] =
      (frame_data.cont_read.no_of_data / 16) + '0';
 ...
 */
static int8_t xgb_prepare_continuous_write_frame(
    u_frame *frame, const cmd_frame_data frame_data) {
  // prepare message - fill union with 0s
  u_frame temp_frame = {0};
  ...
  temp_frame.cont_write_frame.station_number[0] =
      (frame_data.cont_write.station_number / 16) + '0';
  ...
  temp_frame.cont_write_frame.device_lenght[0] =
      ((3 + strlen(frame_data.cont_write.p_device_address)) / 16) + '0';
  ...
  temp_frame.cont_write_frame.device_name[1] =
      frame_data.cont_write.device_type;  // device memory group (P,M,L etc.)
  temp_frame.cont_write_frame.device_name[2] = frame_data.cont_write.data_size;
  strcpy((char *restrict)(&temp_frame.cont_write_frame.device_name[3]),
         (const char *)frame_data.cont_write.p_device_address);

  // no of data
  temp_frame.cont_write_frame.no_data[0] =
      (frame_data.cont_write.no_of_data / 16) + '0';
  temp_frame.cont_write_frame.no_data[1] =
      (frame_data.cont_write.no_of_data % 16) + '0';

  // prepare frame data
    uint8_t no_bytes_to_copy =
        frame_data.cont_write.no_of_data *
        (data_marking_to_size(frame_data.cont_write.data_size) * 2);

    memcpy(temp_frame.cont_write_frame.data, frame_data.cont_write.p_data_buffer,
           no_bytes_to_copy);
 ...
 */
uint8_t xgb_send_individual_write_cmd(const cmd_frame_data frame_data) {
  u_frame frame;

  if (xgb_prepare_individual_write_frame(&frame, frame_data) != XGB_OK) {
    return XGB_ERR_EOT_MISSING;
  }
...
uint8_t xgb_send_continuous_read_cmd(const cmd_frame_data frame_data) {
  ...
  if (xgb_prepare_continuous_read_frame(&frame, frame_data) != XGB_OK) {
    return XGB_ERR_EOT_MISSING;
  }
...
uint8_t xgb_send_continuous_write_cmd(const cmd_frame_data frame_data) {
  ...
  if (xgb_prepare_continuous_write_frame(&frame, frame_data) != XGB_OK) {
    return XGB_ERR_EOT_MISSING;
  }
