 ...
 * 		Some functions need other functionalities. It should works
 * automatically.
 *
 * 		1 - will be compiled
 * 		0 - won't be compiled
...
#endif
#if (USING_RECTANGLE == 0) && (USING_FILL_RECTANGLE == 0) &&                   \
    (USING_FILL_CIRCLE == 0) && (USING_ROUND_RECTANGLE == 0) &&                \
    (USING_TRIANGLE == 0) && (USING_FILL_TRIANGLE == 0)
...
#endif
#if (USING_RECTANGLE == 1) || (USING_FILL_RECTANGLE == 1) ||                   \
    (USING_FILL_CIRCLE == 1) || (USING_ROUND_RECTANGLE == 1) ||                \
    (USING_TRIANGLE == 1) || (USING_FILL_TRIANGLE == 1)
...
#if USING_LINES == 1
void GFX_DrawLine(int x_start, int y_start, int x_end, int y_end,
                  ColorType color);
#endif
...
void GFX_DrawFillRectangle(int x, int y, uint16_t w, uint16_t h,
                           ColorType color);
#endif
#if USING_CIRCLE == 1
...
#endif
#if USING_ROUND_RECTANGLE == 1
void GFX_DrawRoundRectangle(int x, int y, uint16_t w, uint16_t h, uint16_t r,
                            ColorType color);
#endif
#if USING_FILL_ROUND_RECTANGLE == 1
void GFX_DrawFillRoundRectangle(int x, int y, uint16_t w, uint16_t h,
                                uint16_t r, ColorType color);
#endif
#if USING_TRIANGLE == 1
void GFX_DrawTriangle(int x0, int y0, int x1, int y1, int x2, int y2,
                      ColorType color);
#endif
#if USING_FILL_TRIANGLE == 1
void GFX_DrawFillTriangle(int x0, int y0, int x1, int y1, int x2, int y2,
                          ColorType color);
#endif
#if USING_IMAGE == 1
#if AVR_USING == 1
void GFX_Image_P(int x, int y, uint8_t *img, uint8_t w, uint8_t h,
                 ColorType color);
#endif
#if STM32_USING == 1
void GFX_Image(int x, int y, const uint8_t *img, uint16_t w, uint16_t h);
#if USING_IMAGE_ROTATE == 1
void GFX_ImageRotate(int x, int y, const uint8_t *img, uint8_t w, uint8_t h,
                     ColorType color, uint16_t angle);
#endif
#endif
#endif
